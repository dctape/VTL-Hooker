...
static bool global_exit; // Permet d'arrêter le programme
...
//Fonction pour le traitement des paquets reçus
...
{	
	...
	printf("arrêt de rx_and_process\n");

}

//TODO : à supprimer ou à remplacer...
...
int main(int argc, char **argv)
{
	...
	struct config cfg = { // TODO: à modifier...
		.ifindex   = -1, //TODO : provoque un segmentation fault
		.do_unload = false,
		.filename = "",
		.progsec = "xdp_sock"
	};
	...
	/* TODO: code temporaire pour le chargement de programme xdp */
	int xsks_map_fd;
	...
	printf("before if_nametoindex\n");

	cfg.ifindex = if_nametoindex(cfg.ifname);

	printf("after if_nametoindex - cfg.ifindex : %d\n",cfg.ifindex);
	...
	printf("after posix_memalign\n");

	/* Initialize shared packet_buffer for umem usage */
	umem = configure_xsk_umem(packet_buffer, packet_buffer_size);
	if (umem == NULL) {

		printf("Err : umem = NULL\n");
	...
	printf("after configure_xsk_umem\n");

	/* Open and configure the AF_XDP (xsk) socket */
	xsk_socket = xsk_configure_socket(&cfg, umem);
	if (xsk_socket == NULL) {

		printf("Err : xsk_socket = NULL\n");
	...
	printf("after xsk_configure_socket\n");
